...
FormData$1.prototype._lengthRetriever = function(value, callback) {
  if (Object.prototype.hasOwnProperty.call(value, "fd")) {
    if (value.end != void 0 && value.end != Infinity && value.start != void 0) {
      ...
      fs$j.stat(value.path, function(err, stat2) {
        var fileSize;
        if (err) {
          callback(err);
          return;
...
isFetchSupported && ((res) => {
  ["text", "arrayBuffer", "blob", "formData", "stream"].forEach((type2) => {
    !resolvers[type2] && (resolvers[type2] = utils$3.isFunction(res[type2]) ? (res2) => res2[type2]() : (_, config) => {
...
async function createMainWindow() {
  mainWindow = new require$$1$5.BrowserWindow({
    ...
    webPreferences: {
      ...
      // Adjusted path assuming this file is in src/main/
...
function showAuthWindow(authPageUrl, preloadPath) {
  ...
  authWindow = new require$$1$5.BrowserWindow({
    width: 420,
    height: 640,
    show: true,
    webPreferences: {
...
async function storeTokenObject(tokenObject) {
  ...
  const completeTokenObject = {
    ...
    obtainedAt: tokenObject.obtainedAt || Date.now()
    // Default if not provided
  };
...
async function getStoredTokenObject() {
  try {
    ...
    console.error("[AuthService] Error retrieving/parsing token object from keychain:", error2);
    try {
...
async function clearStoredTokens() {
  try {
    ...
    console.error("[AuthService] Error deleting tokens from keychain:", error2);
    throw error2;
  }
}
...
async function attemptTokenRefresh(_handleSignOutUICallback) {
  ...
  try {
    const response = await axios.post(CLERK_TOKEN_ENDPOINT, new node_url.URLSearchParams({
      ...
      // USE AppClerkClientId from config
    ...
    console.error("[AuthService] ##### ERROR refreshing token #####:", error2.isAxiosError ? error2.toJSON() : error2);
    if (error2.response && (error2.response.status === 400 || error2.response.status === 401 || error2.response.status === 403)) {
...
async function exchangeCodeForTokensAndStore(oneTimeCode, apiBaseUrl) {
  var _a;
  try {
    ...
    return { success: true, userId, newAccessToken: accessToken };
  } catch (error2) {
    console.error("[AuthService] ##### ERROR exchanging OTC or storing tokens #####:", error2.isAxiosError ? error2.toJSON() : error2);
    let errorMessage = "Failed to complete sign in during token exchange.";
    if (error2.response) {
      console.error("[AuthService] OTC Exchange Error response data:", error2.response.data);
      errorMessage = ((_a = error2.response.data) == null ? void 0 : _a.message) || errorMessage;
    }
    return { success: false, error: errorMessage, userId: null, newAccessToken: null };
  }
}
let _handleSignOut;
let _triggerRestoreLatestGood;
let tray = null;
function getTrayIconPath() {
  let iconPath;
  const iconFileName = process.platform === "darwin" ? "trayTemplate.png" : "icon.png";
  if (require$$1$5.app.isPackaged) {
    const base = path$n.dirname(require$$1$5.app.getPath("exe"));
    let platformSpecificIconName = iconFileName;
    if (process.platform === "win32") {
      const icoPath = path$n.join(base, "assets", "icon.ico");
      if (fs$k.existsSync(icoPath)) {
        platformSpecificIconName = "icon.ico";
      }
    }
    iconPath = path$n.join(base, "assets", platformSpecificIconName);
    if (!fs$k.existsSync(iconPath)) {
      iconPath = path$n.join(process.resourcesPath, "assets", platformSpecificIconName);
      if (process.platform === "win32" && platformSpecificIconName === "icon.ico" && !fs$k.existsSync(iconPath)) {
        const pngPathInResources = path$n.join(process.resourcesPath, "assets", "icon.png");
        if (fs$k.existsSync(pngPathInResources)) iconPath = pngPathInResources;
      }
    }
  } else {
    iconPath = path$n.join(__dirname, "../../assets", iconFileName);
    if (process.platform === "win32") {
      const icoDevPath = path$n.join(__dirname, "../../assets", "icon.ico");
      if (fs$k.existsSync(icoDevPath)) {
        iconPath = icoDevPath;
      }
    }
...
function loadNativeImage(iconPath) {
  if (!fs$k.existsSync(iconPath)) {
    console.error(`[TrayManager] Icon file NOT FOUND at: ${iconPath}`);
    if (process.platform === "darwin") {
      const systemIcon = require$$1$5.nativeImage.createFromNamedImage("NSActionTemplate", [-1, -1, -1, -1]);
      if (systemIcon && !systemIcon.isEmpty()) return systemIcon;
    }
    return require$$1$5.nativeImage.createEmpty();
  }
  try {
    const img = require$$1$5.nativeImage.createFromPath(iconPath);
...
async function updateTrayMenu() {
  ...
  if (!_handleSignOut) {
    ...
    const minimalMenu = require$$1$5.Menu.buildFromTemplate([
      { label: "Quit PageLifeline", click: () => require$$1$5.app.quit() }
    ]);
    tray.setContextMenu(minimalMenu);
    return;
  }
  const accessToken = await getStoredAccessToken(_handleSignOut);
  let menuTemplate;
  if (accessToken) {
    menuTemplate = [
      {
    ...
    menuTemplate = [
      {
        ...
        click: () => {
          ...
          const preloadPath = path$n.join(__dirname, "../preload.js");
          showAuthWindow(authPageUrl, preloadPath);
        }
      },
      // { label: 'Restore latest good snapshot', enabled: false, click: _legacyRestoreLatestWithId }, // Keep if needed for other restore types 
      { label: "Restore Latest Good Snapshot", enabled: false },
  ...
  const currentContextMenu = require$$1$5.Menu.buildFromTemplate(menuTemplate);
  tray.setContextMenu(currentContextMenu);
  console.log("[TrayManager] Tray context menu updated. Signed in status:", !!accessToken);
}
async function initTray(handleSignOutCallback, triggerRestoreLatestGoodCallback) {
  ...
  try {
    ...
    tray = new require$$1$5.Tray(image);
...
function registerProtocol() {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      require$$1$5.app.setAsDefaultProtocolClient(APP_PROTOCOL, process.execPath, [path$n.resolve(process.argv[1])]);
    }
  } else {
    require$$1$5.app.setAsDefaultProtocolClient(APP_PROTOCOL);
  }
  console.log(`[AppLifecycle] Registered protocol client for ${APP_PROTOCOL}`);
}
function requestSingleInstanceLock(handleOAuthCallback2) {
  _handleOAuthCallback = handleOAuthCallback2;
  const gotTheLock = require$$1$5.app.requestSingleInstanceLock();
  if (!gotTheLock) {
    require$$1$5.app.quit();
  } else {
    require$$1$5.app.on("second-instance", (event, commandLine) => {
      ...
      if (urlOpened) {
        ...
        if (mainWindow2 && mainWindow2.isMinimized()) mainWindow2.restore();
        if (mainWindow2) mainWindow2.focus();
      } else if (mainWindow2) {
...
function initializeOpenUrlHandler(handleOAuthCallback2) {
  if (!_handleOAuthCallback) _handleOAuthCallback = handleOAuthCallback2;
  require$$1$5.app.on("open-url", (event, url2) => {
    console.log(`[AppLifecycle] open-url event. URL: ${url2}`);
    event.preventDefault();
    deeplinkingUrl = void 0;
    if (_handleOAuthCallback) {
      _handleOAuthCallback(url2);
    } else {
...
function getInitialDeeplinkUrl() {
  let initialUrl = void 0;
  if (process.platform !== "darwin") {
    initialUrl = deeplinkingUrl;
  }
  const argvUrl = process.argv.find((arg) => arg.startsWith(`${APP_PROTOCOL}://`));
  if (argvUrl) initialUrl = argvUrl;
  if (initialUrl) deeplinkingUrl = void 0;
  return initialUrl;
}
function initializeWindowAllClosedHandler() {
...
(function(module2, exports) {
  (function(global2) {
    ...
    if (AbortController2 == void 0) {
      ...
      fetch2 = function(url2, options) {
        var signal = options.signal;
        return originalFetch2(url2, { headers: options.headers, credentials: options.credentials, cache: options.cache }).then(function(response) {
    ...
    TextDecoderPolyfill.prototype.decode = function(octets) {
      function valid2(codePoint2, shift, octetsCount2) {
        if (octetsCount2 === 1) {
      ...
      for (var i = 0; i < octets.length; i += 1) {
        var octet = octets[i];
        if (bitsNeeded !== 0) {
          if (octet < 128 || octet > 191 || !valid2(codePoint << 6 | octet & 63, bitsNeeded - 6, octetsCount(bitsNeeded, codePoint))) {
            bitsNeeded = 0;
            codePoint = REPLACER;
        ...
        if (bitsNeeded === 0) {
          ...
          if (bitsNeeded !== 0 && !valid2(codePoint, bitsNeeded, octetsCount(bitsNeeded, codePoint))) {
            bitsNeeded = 0;
            codePoint = REPLACER;
          }
        } else {
    ...
    XHRWrapper.prototype.open = function(method, url2) {
      this._abort(true);
      var that = this;
      var xhr = this._xhr;
      var state2 = 1;
      ...
      var onFinish = function(type2, event) {
      ...
      if ("contentType" in xhr || !("ontimeout" in XMLHttpRequest2.prototype)) {
        url2 += (url2.indexOf("?") === -1 ? "?" : "&") + "padding=true";
      }
    ...
    XHRTransport.prototype.open = function(xhr, onStartCallback, onProgressCallback, onFinishCallback, url2, withCredentials, headers) {
      xhr.open("GET", url2);
      var offset = 0;
    ...
    FetchTransport.prototype.open = function(xhr, onStartCallback, onProgressCallback, onFinishCallback, url2, withCredentials, headers) {
      var reader = null;
    ...
    EventTarget.prototype.addEventListener = function(type2, listener) {
      type2 = String(type2);
      var listeners = this._listeners;
      var typeListeners = listeners[type2];
      if (typeListeners == void 0) {
        typeListeners = [];
        listeners[type2] = typeListeners;
      }
      var found = false;
    ...
    EventTarget.prototype.removeEventListener = function(type2, listener) {
      type2 = String(type2);
      var listeners = this._listeners;
      var typeListeners = listeners[type2];
      if (typeListeners != void 0) {
    ...
    function MessageEvent(type2, options) {
      Event.call(this, type2);
      this.data = options.data;
      this.lastEventId = options.lastEventId;
    }
    MessageEvent.prototype = Object.create(Event.prototype);
    function ConnectionEvent(type2, options) {
      Event.call(this, type2);
    ...
    function ErrorEvent(type2, options) {
      Event.call(this, type2);
    ...
    function EventSourcePolyfill(url2, options) {
    ...
    var isFetchSupported2 = fetch2 != void 0 && Response2 != void 0 && "body" in Response2.prototype;
    function start(es, url2, options) {
      url2 = String(url2);
      ...
      var xhr = isFetchSupported2 && TransportOption == void 0 ? void 0 : new XHRWrapper(TransportOption != void 0 ? new TransportOption() : getBestXHRTransport());
          ...
          for (var position = 0; position < chunk.length; position += 1) {
            ...
            if (state2 === AFTER_CR && c === "\n".charCodeAt(0)) {
              ...
              if (c === "\r".charCodeAt(0) || c === "\n".charCodeAt(0)) {
                ...
                state2 = c === "\r".charCodeAt(0) ? AFTER_CR : FIELD_START;
              } else {
                if (state2 === FIELD_START) {
                  fieldStart = position;
                  state2 = FIELD;
      ...
      var onTimeout = function() {
        ...
        state2 = FIELD_START;
        var requestURL = url2;
        if (url2.slice(0, 5) !== "data:" && url2.slice(0, 5) !== "blob:") {
          if (lastEventId !== "") {
            var i = url2.indexOf("?");
            requestURL = i === -1 ? url2 : url2.slice(0, i + 1) + url2.slice(i + 1).replace(/(?:^|&)([^=&]*)(?:=[^&]*)?/g, function(p, paramName) {
              return paramName === lastEventIdQueryParameterName ? "" : p;
            });
            requestURL += (url2.indexOf("?") === -1 ? "?" : "&") + lastEventIdQueryParameterName + "=" + encodeURIComponent(lastEventId);
          }
        }
...
let _mainHandleSignOut;
let _legacyRestoreFn;
let _API_BASE_URL;
const activeSseConnections = {};
function initDependencies(apiBaseUrl, handleSignOut2, legacyRestoreCallback) {
  _API_BASE_URL = apiBaseUrl;
  _mainHandleSignOut = handleSignOut2;
  _legacyRestoreFn = legacyRestoreCallback;
}
function closeSseConnection(restoreJobId) {
...
async function performRestoreLatestGood() {
  var _a, _b, _c, _d;
  console.log("[IPCHandlers-Core] Attempting to restore latest good snapshot.");
  const accessToken = await getStoredAccessToken(_mainHandleSignOut);
  if (!accessToken) {
    new require$$1$5.Notification({ title: "Authentication Required", body: "Please sign in to restore." }).show();
  ...
  try {
    ...
    if ((backendData == null ? void 0 : backendData.success) && backendData.restoreJobId) {
      new require$$1$5.Notification({
      ...
      const es = new eventsourceExports.EventSourcePolyfill(sseUrl, { headers: { "Authorization": `Bearer ${accessToken}` } });
      activeSseConnections[restoreJobId] = es;
      es.onopen = () => {
        console.log(`[IPCHandlers-SSE] Connection opened for restoreJobId: ${restoreJobId}`);
        mainWindow2 == null ? void 0 : mainWindow2.webContents.send("restore-progress-update", { type: "sse_connected", restoreJobId });
      };
      es.onmessage = (event) => {
        try {
          const progressData = JSON.parse(event.data);
          console.log(`[IPCHandlers-SSE] Message for ${restoreJobId}:`, progressData);
          mainWindow2 == null ? void 0 : mainWindow2.webContents.send("restore-progress-update", { type: "progress", restoreJobId, data: progressData });
          if (progressData.status === "completed" || progressData.status === "error") {
            closeSseConnection(restoreJobId);
            if (progressData.status === "completed" && progressData.restoredPageUrl) {
              mainWindow2 == null ? void 0 : mainWindow2.webContents.send("restore-progress-update", { type: "completed_with_url", restoreJobId, url: progressData.restoredPageUrl, message: progressData.message });
            }
          }
        } catch (parseError) {
      ...
      es.onerror = (errorEvent) => {
        ...
        mainWindow2 == null ? void 0 : mainWindow2.webContents.send("restore-progress-update", { type: "sse_error", restoreJobId, error: "SSE connection error" });
        closeSseConnection(restoreJobId);
      };
    } else {
      new require$$1$5.Notification({
        title: "PageLifeline: Restore Failed",
        body: (backendData == null ? void 0 : backendData.message) || "Could not initiate restore of latest good snapshot."
      }).show();
    }
    return backendData;
  } catch (error2) {
    console.error("[IPCHandlers-Core] Error calling /api/restore/latest-good:", ((_a = error2.response) == null ? void 0 : _a.data) || error2.message);
    let friendlyMessage = "Failed to initiate latest good restore.";
    if ((_c = (_b = error2.response) == null ? void 0 : _b.data) == null ? void 0 : _c.message) {
      friendlyMessage = error2.response.data.message;
    } else if (error2.isAxiosError && !error2.response) {
      friendlyMessage = "Network error. Please check your connection.";
    }
    new require$$1$5.Notification({ title: "PageLifeline: Error", body: friendlyMessage }).show();
    return {
      success: false,
      message: friendlyMessage,
      errorDetails: (_d = error2.response) == null ? void 0 : _d.data
    };
  }
}
function registerAuthEventHandlers() {
  require$$1$5.ipcMain.on("clerk-auth-success", async (event, { sessionId, token }) => {
    ...
    if (token) {
      try {
        ...
        await storeTokenObject(tokensToStore);
        new require$$1$5.Notification({ title: "PageLifeline: Signed In", body: "Successfully signed in." }).show();
        await ensureMainWindowVisibleAndFocused();
      } catch (keytarError) {
        new require$$1$5.Notification({ title: "PageLifeline: Sign In Error", body: keytarError.message || "Could not save session." }).show();
      }
    }
    closeAuthWindow();
  });
  require$$1$5.ipcMain.on("clerk-auth-error", (event, { error: error2 }) => {
    console.error("[IPCHandlers] clerk-auth-error:", error2);
    new require$$1$5.Notification({ title: "PageLifeline: Sign In Failed", body: error2 || "Unknown sign-in error." }).show();
    closeAuthWindow();
  });
  require$$1$5.ipcMain.handle("get-auth-status", async () => {
  ...
  require$$1$5.ipcMain.on("request-sign-in", () => {
...
function registerSnapshotAndRestoreHandlers() {
  require$$1$5.ipcMain.handle("get-snapshots", async (_event, isRetry = false) => {
    const accessToken = await getStoredAccessToken(_mainHandleSignOut);
    if (!accessToken) {
      return [];
    }
    try {
      ...
      if (e.response && e.response.status === 401 && !isRetry) {
        const newAccessToken = await attemptTokenRefresh(_mainHandleSignOut);
        if (newAccessToken) {
          const thisHandler = require$$1$5.ipcMain.listeners("get-snapshots")[0];
          return thisHandler(_event, true);
        } else {
          await _mainHandleSignOut();
        }
  ...
  require$$1$5.ipcMain.on("restore-latest", async (_event, payload) => {
    const snapshotId = payload == null ? void 0 : payload.snapshotId;
    const result = await _legacyRestoreFn(snapshotId);
    const mainWindow2 = getMainWindow();
    if (mainWindow2) {
      mainWindow2.webContents.send("restore-result", result);
    }
  });
  require$$1$5.ipcMain.handle("create-test-snapshot", async (_event, isRetry = false) => {
    const accessToken = await getStoredAccessToken(_mainHandleSignOut);
    if (!accessToken) {
    ...
    try {
      ...
      if (e.response && e.response.status === 401 && !isRetry) {
        const newAccessToken = await attemptTokenRefresh(_mainHandleSignOut);
        if (newAccessToken) {
          const thisHandler = require$$1$5.ipcMain.listeners("create-test-snapshot")[0];
          return thisHandler(_event, true);
        } else {
          await _mainHandleSignOut();
        }
  ...
  require$$1$5.ipcMain.handle("get-snapshot-download-url", async (_event, snapshotId, isRetry = false) => {
    const accessToken = await getStoredAccessToken(_mainHandleSignOut);
    if (!accessToken) {
      return { success: false, error: "No JWT" };
    }
    try {
      ...
      if (e.response && e.response.status === 401 && !isRetry) {
        const newAccessToken = await attemptTokenRefresh(_mainHandleSignOut);
        if (newAccessToken) {
          const thisHandler = require$$1$5.ipcMain.listeners("get-snapshot-download-url")[0];
  ...
  require$$1$5.ipcMain.handle("restore-latest-good", async () => {
    console.log("[IPCHandlers] IPC 'restore-latest-good' received. Calling core logic.");
    return await performRestoreLatestGood();
  });
}
function registerAllIpcHandlers(apiBaseUrl, handleSignOut2, legacyRestoreCallback) {
  initDependencies(apiBaseUrl, handleSignOut2, legacyRestoreCallback);
...
if (!fs$h[gracefulQueue]) {
  ...
  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || "")) {
    process.on("exit", function() {
      ...
      require$$4$1.equal(fs$h[gracefulQueue].length, 0);
    });
  }
}
...
function initAutoUpdater() {
  ...
  main$2.autoUpdater.autoInstallOnAppQuit = true;
  ...
  main$2.autoUpdater.on("checking-for-update", () => {
    ...
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "checking-for-update" });
  });
  main$2.autoUpdater.on("update-available", (info) => {
    var _a;
    log.info("[Updater] Update available:", info);
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "update-available", data: info });
  });
  main$2.autoUpdater.on("update-not-available", (info) => {
    var _a;
    log.info("[Updater] Update not available.", info);
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "update-not-available", data: info });
  });
  main$2.autoUpdater.on("error", (err) => {
    var _a;
    log.error("[Updater] Error in auto-updater:", err);
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "error", data: err.message });
  });
  main$2.autoUpdater.on("download-progress", (progressObj) => {
    var _a;
    log.info(`[Updater] Download progress: ${progressObj.percent}%`);
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "download-progress", data: progressObj });
  });
  main$2.autoUpdater.on("update-downloaded", (info) => {
    var _a;
    log.info("[Updater] Update downloaded. Will quit and install on restart.", info);
    (_a = getMainWindowForUpdater()) == null ? void 0 : _a.webContents.send("updater-event", { event: "update-downloaded", data: info });
  });
  setTimeout(() => {
...
require$$1$5.ipcMain.on("updater-download-update", () => {
  log.info("[Updater IPC] Received request to download update.");
  main$2.autoUpdater.downloadUpdate();
});
require$$1$5.ipcMain.on("updater-quit-and-install", () => {
...
async function handleSignOut() {
  try {
    ...
    new require$$1$5.Notification({ title: "PageLifeline: Signed Out", body: "Successfully signed out." }).show();
    await updateTrayMenu();
    const mainWindow2 = getMainWindow();
    if (mainWindow2 && !mainWindow2.isDestroyed()) {
      mainWindow2.webContents.send("user-signed-out");
    }
  } catch (error2) {
    console.error("[Main.ts] Error during sign out process:", error2);
    new require$$1$5.Notification({ title: "PageLifeline: Sign Out Error", body: "Could not sign out properly." }).show();
    await updateTrayMenu();
  }
}
async function restoreLatest(snapshotId, isRetry = false) {
  ...
  const accessToken = await getStoredAccessToken(handleSignOut);
  if (!accessToken) {
    new require$$1$5.Notification({ title: "PageLifeline: Authentication Required", body: "Please sign in first." }).show();
    if (!isRetry) await updateTrayMenu();
    return { success: false, message: "Authentication required." };
  }
  if (!snapshotId) {
    new require$$1$5.Notification({ title: "PageLifeline: Restore Failed", body: "No snapshot selected." }).show();
  ...
  try {
    ...
    new require$$1$5.Notification({ title: "PageLifeline: Restore Initiated", body: response.data.message || "...progress." }).show();
    ...
    if (err.response) {
      ...
      if (err.response.status === 401 && !isRetry) {
        const newAccessToken = await attemptTokenRefresh(handleSignOut);
        if (newAccessToken) return restoreLatest(snapshotId, true);
        else {
          errorMessage = "Auth failed. Sign in again.";
          await handleSignOut();
        }
      } else if (err.response.status === 401 && isRetry) {
        errorMessage = "Auth failed after retry. Sign in again.";
        await handleSignOut();
      }
    }
    new require$$1$5.Notification({ title: "PageLifeline: Restore Failed", body: errorMessage }).show();
...
async function handleOAuthCallback(callbackUrl) {
  ...
  try {
    const parsedUrl = new node_url.URL(callbackUrl);
    ...
    if (error2) {
      ...
      console.error(`[MainApp] handleOAuthCallback: Error from relay: ${error2}, Description: ${errorDesc}`);
      new require$$1$5.Notification({ title: "Auth Error", body: `Failed: ${errorDesc || error2}` }).show();
      return;
    }
    if (!oneTimeCode) {
      console.error("[MainApp] handleOAuthCallback: One-Time Code not found in callback URL from relay.");
      new require$$1$5.Notification({ title: "Auth Error", body: "Could not get one-time code." }).show();
      return;
    }
    console.log(`[MainApp] handleOAuthCallback: Extracted One-Time Code: ${oneTimeCode}`);
    const authResult = await exchangeCodeForTokensAndStore(oneTimeCode, API_BASE_URL);
    if (authResult.success && authResult.newAccessToken) {
      new require$$1$5.Notification({ title: "Authentication Successful!", body: "You are now signed in." }).show();
      await ensureMainWindowVisibleAndFocused();
      await updateTrayMenu();
    } else {
    ...
    console.error("[MainApp] handleOAuthCallback: ##### OVERALL ERROR #####:", error2.message);
    new require$$1$5.Notification({ title: "Auth Error", body: error2.message || "Sign in failed. Check logs." }).show();
  }
}
if (!requestSingleInstanceLock(handleOAuthCallback)) {
  require$$1$5.app.quit();
}
initializeOpenUrlHandler(handleOAuthCallback);
initializeWindowAllClosedHandler();
registerAllIpcHandlers(API_BASE_URL, handleSignOut, restoreLatest);
require$$1$5.app.whenReady().then(async () => {
  console.log("[AppEvent] App is ready.");
  await createMainWindow();
  await initTray(handleSignOut, restoreLatest);
  registerAllIpcHandlers(API_BASE_URL, handleSignOut, restoreLatest);
  initAutoUpdater();
  console.log("[AppEvent] Core initializations complete (Window, Tray, IPC Handlers, Lifecycle, Updater).");
  const initialUrl = getInitialDeeplinkUrl();
  if (initialUrl) {
