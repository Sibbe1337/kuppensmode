...
var restoreTrigger = (0, import_functions_framework.http)("restoreTrigger", async (req, res) => {
  ...
  try {
    ...
    try {
      await restoreJobRef.update({ status: "PROCESSING", updatedAt: import_firestore3.Timestamp.now() });
      ...
      do {
        const response = await notion.blocks.children.list({ block_id: pageId, start_cursor: nextCursor ?? void 0 });
      ...
      await restoreJobRef.update({ status: "COMPLETED", updatedAt: import_firestore3.Timestamp.now() });
