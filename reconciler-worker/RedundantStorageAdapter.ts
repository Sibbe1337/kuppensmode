...
export class RedundantStorageAdapter implements StorageAdapter {
  private primary: StorageAdapter;
  private mirrors: StorageAdapter[];
  private defaultRetryAttempts = 1; // Minimal retry (1 extra attempt)
  ...
  private async attemptWithRetry<T>(
    ...
    try {
      ...
      if (attempts > 0 && isRetryableError(error)) {
        console.warn(`Retrying operation due to retryable error: ${(error as Error).message}. Attempts left: ${attempts}`);
  ...
  async write(
    ...
    const results = await Promise.allSettled(
      allAdapters.map(adapter => 
        this.attemptWithRetry(() => adapter.write(path, data, metadata))
      )
    );
  ...
  async read(path: string): Promise<Buffer> {
    try {
      ...
      console.warn(`Primary adapter failed to read ${path}: ${(primaryError as Error).message}. Trying mirrors.`);
      for (const mirror of this.mirrors) {
        try {
          return await this.attemptWithRetry(() => mirror.read(path));
        } catch (mirrorError) {
          console.warn(`Mirror adapter failed to read ${path}: ${(mirrorError as Error).message}. Trying next.`);
        }
      }
      throw new Error(`Failed to read ${path} from primary and all mirrors. Last primary error: ${(primaryError as Error).message}`);
    }
  }
  ...
  async list(pathPrefix: string): Promise<string[]> {
    ...
    try {
      return await this.attemptWithRetry(() => this.primary.list(pathPrefix));
        ...
        console.warn(`Primary adapter failed to list ${pathPrefix}: ${(primaryError as Error).message}. Trying mirrors if available.`);
        if (this.mirrors.length > 0) {
            for (const mirror of this.mirrors) {
                try {
                    return await this.attemptWithRetry(() => mirror.list(pathPrefix));
                } catch (mirrorError) {
                    console.warn(`Mirror adapter failed to list ${pathPrefix}: ${(mirrorError as Error).message}. Trying next.`);
                }
            }
        }
        throw new Error(`Failed to list ${pathPrefix} from primary and all mirrors. Last primary error: ${(primaryError as Error).message}`);
    }
  }
  ...
  async delete(path: string): Promise<void> {
    ...
    const results = await Promise.allSettled(
      allAdapters.map(adapter => 
        this.attemptWithRetry(() => adapter.delete(path), 0) // 0 retries for delete for fire-and-forget
      )
    );
  ...
  async exists(path: string): Promise<boolean> {
    try {
      ...
      console.warn(`Primary adapter failed on exists for ${path}: ${(primaryError as Error).message}. Trying mirrors.`);
    }
    for (const mirror of this.mirrors) {
      try {
        ...
        console.warn(`Mirror adapter failed on exists for ${path}: ${(mirrorError as Error).message}. Trying next.`);
      }
    }
    return false;
  }
  ...
  async getMetadata(path: string): Promise<Record<string, any> | null> {
    ...
    if (this.primary.getMetadata) {
        try {
            ...
            console.warn(`Primary adapter failed getMetadata for ${path}: ${(primaryError as Error).message}. Trying mirrors.`);
            for (const mirror of this.mirrors) {
                if (mirror.getMetadata) {
                    try {
                        return await this.attemptWithRetry(() => mirror.getMetadata!(path));
                    } catch (mirrorError) {
                        console.warn(`Mirror adapter failed getMetadata for ${path}: ${(mirrorError as Error).message}. Trying next.`);
                    }
                }
            }
        }
  ...
  async copy(srcPath: string, destPath: string): Promise<void> {
    ...
    try {
        ...
        console.error(`Failed to copy ${srcPath} to ${destPath} on primary adapter: ${(e as Error).message}`);
        throw e;
    }
  }
} 