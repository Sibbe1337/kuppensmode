...
export default function AnalyticsDashboardPage() {
  ...
  const dummyMonthlyUsage = quotaData ? 
  ...
  // Map Snapshot[] to SnapshotEntry[] for ActivityLog
  const activityLogSnapshots = snapshotsData?.map(snap => ({
    id: snap.id,
    name: snap.snapshotIdActual || snap.id, // Use snapshotIdActual or fallback to full ID as name
    date: snap.timestamp, // ActivityLog's formatDate will handle this ISO string
    sizeKb: snap.sizeKB,
  })) || [];
    ...
    <main className="container mx-auto py-8 px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
        ...
        {quotaData && (
            <KpiCard 
                title="Monthly Usage" 
                value={`${(quotaData.snapshotsUsed / quotaData.snapshotsLimit * 100).toFixed(0)}%`} 
                subtitle="Snapshot storage and processing" 
                gradientPreset="cyan" 
                slotRight={<MonthlyUsageGauge value={quotaData.snapshotsUsed} limit={quotaData.snapshotsLimit} warning={quotaData.snapshotsUsed >= quotaData.snapshotsLimit * 0.85} />}
            />
        )}
      </div>
